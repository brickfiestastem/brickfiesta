...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        obj_output_image = Image.new(
            'RGB', (self.int_label_width, self.int_label_height), color='white')
        obj_output_draw = ImageDraw.Draw(obj_output_image)
        int_width, int_height = obj_output_image.size
        int_text_width, int_text_height = obj_output_draw.textsize(
            options['center_text'], self.font_text)

        obj_barcode_image = Image.open("{}.png".format(self.tmp_image))
        obj_barcode_image.thumbnail(
            (self.int_label_width, self.int_label_width), Image.ANTIALIAS)
        ...
        obj_output_draw.text(
            (math.floor((int_width - int_text_width) / 2),
             int_height - int_text_height - 5),
            options['center_text'],
